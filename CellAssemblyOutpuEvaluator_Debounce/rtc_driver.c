...
/*
	...
	stRtcRegs.mHour  	= 0x15;							// hour = 21 ,24-hour mode(bit 6=0)
...
*/
#ifdef TIME_DEBUG
	stRtcRegs.mSeconds	= 59;							// second = 00
	stRtcRegs.mMinute  	= 30;							// minute = 29
	stRtcRegs.mHour  	= 12;
#else
	// Setup task to run
	...
	stRtcRegs.mMinute  	= 0;							// minute = 29
	stRtcRegs.mHour  	= 0;
	stRtcRegs.mDay		= 0;							// Day = 1 or sunday
	stRtcRegs.mDate		= 0;							// Date = 12
	stRtcRegs.mMonth	= 0;							// month = October
	stRtcRegs.mYear		= 0;	
#endif
...
UINT32 RTC_getSecondCount()
{
	UINT32 secondCount = 
	((UINT32)RTC_getHour() *60*60 ) +
	((UINT32)RTC_getMinute() *60 ) +	
	((UINT32)RTC_getSecond() );
...
#else
	return ConvertBCD2HEX(stRtcRegs.mHour);
#endif
}
...
#else
	return ConvertBCD2HEX(stRtcRegs.mMinute);
#endif
}
...
#else
	return ConvertBCD2HEX(stRtcRegs.mSeconds);
#endif
}
